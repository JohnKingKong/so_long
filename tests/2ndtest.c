...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/18 13:23:11 by jvigneau          #+#    #+#             */
/*   Updated: 2022/01/20 17:03:25 by jvigneau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	tile_wall(t_vars *vars, int xx, int yy)
{
	...
	cnt = 0;
	relative_path = "./download.XPM";
	vars->img.mlx_img = mlx_xpm_file_to_image(vars->mlx, relative_path,
			&img_width, &img_height);
	//while (cnt <= xx)
	{
		mlx_put_image_to_window(vars->mlx, vars->mlx_win,
			vars->img.mlx_img, xx, yy);
		//cnt++;
	}
	return (0);
}
...
int	car(t_vars *vars, int xx, int yy)
{
	...
	carpath = "./car.xpm";
	vars->img.car_img = mlx_new_image(vars->mlx, 64, 64);
	vars->img.car_adr = mlx_get_data_addr(vars->img.car_img, &vars->img.car_bpp,
			&vars->img.car_line_len, &vars->img.car_endian);
	vars->img.mlx_img = mlx_xpm_file_to_image(vars->mlx, carpath,
...
int	handle_keypress(int keysum, t_vars *vars)
{
	...
	{	
		mlx_put_image_to_window(vars->mlx, vars->mlx_win, vars->img.mlx_img,
			0, 0);
		vars->img.pos_car_x = vars->img.pos_car_x + 64;
...
int	main(void){
	...
	/*vars.img.mlx_img = mlx_new_image(vars.mlx, WIN_WIDTH, WIN_HEIGHT);
	vars.img.adr = mlx_get_data_addr(vars.img.mlx_img, &vars.img.bpp,
			&vars.img.line_len, &vars.img.endian);*/
	//tile_wall(&vars, WIN_WIDTH / 2, WIN_HEIGHT / 2);
	//mlx_loop_hook(vars.mlx, &render, &vars);
	mlx_hook(vars.mlx_win, 2, 0, &handle_keypress, &vars);
	mlx_hook(vars.mlx_win, 17, 0, &xox, &vars);
	//mlx_hook(vars.mlx_win, ButtonRelease, ButtonReleaseMask,
	//	&handle_mouse, &vars);
	mlx_loop(vars.mlx);
	free(vars.mlx);
}
